  ...
  def register() = AsyncStack(BodyParsers.parse.json) { implicit request =>
    request.body.validate[SignupRequest] match {
      case s: JsSuccess[SignupRequest] =>
        authService.register(s.get.email, s.get.password, s.get.acceptTerms, nip = s.get.nip).map { user =>
          Ok("")
        }
      case e: JsError =>
        Future.successful(BadRequest(Json.toJson(ErrorWrapper("invalidJson", JsError.toJson(e).toString))))
...
class AuthedAuthController @Inject() (override val authService: AuthService) extends AppController {
  ...
  def changePassword() = AsyncStack(BodyParsers.parse.json, AuthorityKey -> AuthorityImpl()) { implicit request =>
    request.body.validate[ChangePasswordRequest] match {
      case s: JsSuccess[ChangePasswordRequest] =>
        authService.changePasswordForLoggedUser(s.get.oldPassword, s.get.newPassword).map {
          case Right(user) =>
            Ok("ok")
          case Left(error) =>
            BadRequest(Json.toJson(error))
