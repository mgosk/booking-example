...
trait UsersRepository {
  ...
  def remove(userId: UserId):Future[UserId]
...
class UsersRepositoryImpl extends UsersRepository {
  ...
  def collection: JSONCollection = reactiveMongoApi.db.collection[JSONCollection]("auth.users")
  ...
  override def remove(userId: UserId): Future[UserId] = {
    collection.remove(idSelector(userId)).map( lastError =>
      userId
    )
  }
}
